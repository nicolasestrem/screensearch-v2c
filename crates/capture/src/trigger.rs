...
//!
//! - **trailing-edge debounce** — a burst of foreground changes (e.g. tabbing through
...
//!   than `event_min_interval_ms`, so triggers can't storm;
//! - **per-trigger enable gating** — a disabled trigger never fires;
//! - **edge-detected idle** — derived purely from the polled idle time
//!   (`GetLastInputInfo`, never key content), firing once per quiet period.
//!
//! *(0.3.0 PR2 trimmed the six event triggers to foreground + idle; clipboard, click,
//! scroll-stop, and typing-pause were removed — `docs/0.3.0.md`, `02 §5c`.)*
//!
...
/// Idle is *not* here — it is derived from the polled idle time, not pushed as an event.
...
impl TriggerMachine {
    ...
    pub(crate) fn poll(&mut self, now_ms: i64, idle_ms: i64) -> Option<CaptureTrigger> {
        // Re-arm idle once the user is active again (idle dropped back below the
        // threshold), so the next quiet period fires afresh.
        if idle_ms < self.cfg.idle_threshold_ms {
            self.fired_idle = false;
        }

        // `fired_idle` is set only on a SUCCESSFUL emit — if the min-interval rate
...
mod tests {
    ...
    fn min_interval_suppresses_a_second_capture() {
        ...
        m.on_input_event(InputEventKind::Foreground, 1100);
        assert_eq!(
            m.poll(1200, 0),
            Some(CaptureTrigger::ForegroundChange),
    ...
    fn idle_fires_once_per_quiet_period() {
        ...
        assert_eq!(
            ...
            "idle crossed the idle threshold"
        );
        assert_eq!(m.poll(5100, 6000), None, "still quiet → no re-fire");
        assert_eq!(m.poll(8000, 8000), None, "still quiet → no re-fire");
        assert_eq!(m.poll(8100, 0), None, "user active again → re-arms");
        assert_eq!(
            m.poll(14000, 5000),
            Some(CaptureTrigger::Idle),
    ...
    #[test]
    fn idle_disabled_never_emits_from_polling() {
        let mut m = TriggerMachine::new(TriggerConfig {
            on_idle: false,
            ..cfg()
        });
        assert_eq!(m.poll(6000, 6000), None);
        assert_eq!(m.poll(12000, 12000), None);
    }
