...
| 23 | 2026-06-22 | **`toast` event never emitted** — `03 §7` lists `toast` as a core→UI event, but no `KernelEvent::Toast` exists and nothing emits `"toast"`. | **Resolved (agent):** toasts are **client-side** in v1 (driven by command success/failure); the `Toast` ts-rs type is kept for forward-compat. Backend-pushed toasts (e.g. a §12 sidecar-crash fallback) would need a `KernelEvent::Toast` + emit — optional later polish, not a DoD item. | agent | open (optional polish) |
| 24 | 2026-06-22 | **`storage.retention_days` enforcement** — the setting exists and `delete_frame` is a primitive, but no scheduled purge runs. | **Resolved (agent, user-approved):** **persist the value, do not enforce yet** — the Settings screen labels it honestly. A startup/periodic retention sweep is a tracked follow-up (not a v1.0 DoD item). | agent | open (follow-up) |
...
| 27 | 2026-06-22 | **StatusRail "DB size"** — `UI_REFERENCE §3` / the P5 plan list "DB size" as a StatusRail telemetry readout, but no IPC field exposes a database size (the `db` readiness only carries a `ComponentStatus` + a free-text `detail` like "schema v1 (path)"). | **Resolved (agent, M1+M2):** the StatusRail shows the **DB readiness/status** chip (with the `detail` in its tooltip), **not** a fabricated size — fabricating a number is disallowed (CLAUDE.md). A real size readout would need a new command/field (e.g. `db_bytes` from `PRAGMA page_count*page_size` + WAL) — optional follow-up, not a DoD item. | agent | open (optional follow-up) |
| 28 | 2026-06-22 | **Concurrent / cancellable `ask`** — the `answer_delta` event is a single global stream with **no per-request id** (`03 §7`, `src-tauri` `ask`). A second `ask` started while a first is still streaming can't be distinguished on the client, so late deltas from the first would fold into the second's state. | **Mitigated (agent, M1+M2):** `useAsk` blocks a new `ask` while `phase === "streaming"` (and the recall UI will gate its trigger the same way), which removes the race for the single-ask UX. A *true* concurrent or mid-stream-cancellable ask needs a backend change — tag `answer_delta` with a request id (and an `ask`-cancel path) so the client can discard stale deltas. Logged for a later inference pass. | agent | open (needs backend request-id) |
| 29 | 2026-06-22 | **CommandPalette ARIA combobox semantics** — the ⌘K palette uses `<ul>/<button>` (keyboard-navigable) but not the full `role="combobox"` + `aria-activedescendant` + `role="listbox"/"option"` pattern, so screen-reader support is partial. | **Accepted for M1+M2 (agent):** acceptable this phase (local, single-user, sighted-primary context); the keyboard path (↑/↓/Enter/Esc) and visible focus already work. Revisit when the recall search UX lands (M3) and the combobox pattern is genuinely exercised. | agent | open (revisit M3) |
| 30 | 2026-06-22 | **Enrichment-completion data refresh** (Codex review, PR #11) — a completed `embed_*`/`vision_tag` job's only UI signal is `job_progress`, which carries **counts only** (no job kind / frame id). With `refetchOnWindowFocus` off and no polling, an already-cached Moment/Recall/Insights query would never refetch, so a new vision tag or freshly-indexed embedding stays invisible until manual reload. | **Mitigated client-side (agent, M1+M2):** `useLiveEvents` now debounce-invalidates the affected families (`frame`/`search`/`insights`) on `job_progress` — `invalidateQueries` refetches only *observed* queries and marks the rest stale, so an idle backlog drain stays cheap; timeline is excluded (capture density, not enrichment). **Proper fix (deferred, needs backend):** emit a richer completion event (e.g. `job_completed { kind, frame_id }`) so the UI can invalidate surgically instead of by family. | agent | open (needs richer event) |
| 31 | 2026-06-22 | **Frame-browsing query missing** (M3+M4) — the Timeline (hover thumbnails, Enter-opens-a-Moment) and the Deck (jump-back-in) need to map a time/recency onto **real frames**, but the merged M0 backend exposed only density-count buckets (`get_timeline`); `search` requires non-empty text and `untagged_frame_ids` lists only untagged frames — neither returns frame ids + image paths over a window. | **Resolved (user-approved, M3+M4):** added a thin read-only browsing slice mirroring the inherent `get_frame` pattern (no `Store`-trait change): `SqliteStore::frames_in_range(start,end,limit) -> Vec<FrameMeta>` (newest-first over the half-open window) and `nearest_frame(at) -> Option<FrameMeta>` (closest frame on either side; at-or-after wins an exact tie), exposed as `get_frames` / `get_nearest_frame`; new ts-rs `FrameMeta { frame_id, captured_at, image_path, app_hint }`. The Timeline draws thumbnails from `get_frames` and resolves **Enter** to a concrete frame via `get_nearest_frame` (server-side exact, *not* a sampled approximation). Caveat: hover-**preview** thumbnails are chosen from the newest-first window sample, so in a very dense window the preview can bias toward recent frames — the **open** is always exact. | user+agent | ✅ done (M3+M4) |
| 32 | 2026-06-22 | **Timeline `bucket_count` grain** (M3+M4) — `07` #22 made `get_timeline` presentation-driven, but the Timeline passes a **fixed** `bucket_count` (240) rather than one derived from the live ribbon pixel width. | **Resolved (agent, M3+M4):** a constant 240 buckets is ample resolution at any realistic ribbon width and avoids a measure-then-refetch round-trip; the canvas maps the sparse buckets onto the pixel width by time. A width-derived count is an optional later refinement (matters only at extreme widths). | agent | open (optional refinement) |
