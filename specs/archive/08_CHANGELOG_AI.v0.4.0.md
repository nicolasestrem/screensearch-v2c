# 08 — AI Changelog — archived v0.4.0 history

> The 0.4.0 "frames -> sessions" arc (P8): PR1–PR7 build entries, newest first, preserved verbatim.

---

## 2026-07-11 - 0.4.0 PR7: audit + D16 first-auto-delivery gate + tag v0.4.0

- **Change:** the 0.4.0 release audit + sweep. Bumped all four version fields and both lockfiles to
  0.4.0; added the D16 update-path runbook to `docs/TESTING.md`; ran a 10-agent adversarial audit
  confirming D1–D16 landed (`05` Pass 17); passed the D16 first-auto-delivery gate live on a 0.3.3
  install (auto-update -> user-initiated restart -> 10->11 migration -> 9 sessions over pre-existing
  history -> frame features unchanged); swept `05`–`08` and archived the arc. The v0.4.0 tag and
  signed release follow at merge (the D16 gate already passed before the tag, per contract).
- **Why:** `docs/0.4.0.md` §3 PR7 / `03 §13c-7` (the closing audit + release PR).
- **Verification:** full suite green (742 tests / 58 binaries; fmt/clippy/build/bindings clean); the
  D16 log sequence + DB queries are recorded verbatim in `05` Pass 17.
