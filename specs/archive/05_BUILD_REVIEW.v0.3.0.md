# 05 — Build Review — archived v0.3.0 arc (post-0.2.2 → v0.3.0)

> Moved verbatim from the live `specs/05_BUILD_REVIEW.md` on the v0.3.0 release sweep
> (2026-07-04, PR9, `04 §7`). Entries preserved as written.
...
### Release build + installer evidence (0.3.0; log `docs/audits/pr9-installer.log`)
- `npm run tauri build` → `Finished 1 bundle at: …\target\release\bundle\nsis\
  ScreenSearch_0.3.0_x64-setup.exe` (12 883 585 B).
- `7z l ScreenSearch_0.3.0_x64-setup.exe` lists **both** `screensearch.exe` (47 339 008 B) and
  `screensearch-mcp.exe` (3 377 152 B) — §13b.7 installer inclusion.
- Silent install (`/S`) → `%LOCALAPPDATA%\ScreenSearch\` holds `screensearch.exe`,
  `screensearch-mcp.exe`, `uninstall.exe`; `screensearch-mcp.exe --version` →
  **`screensearch-mcp 0.3.0`** (the bumped version, from the installed copy).
- Silent uninstall (`uninstall.exe /S`) → the install dir (including the sidecar) is removed.

### Final gate (§13b.9, branch tip — verbatim tail)
- Recorded in the final commit of this PR: full suite re-run green (fmt --check exit 0; clippy
  `-D warnings` clean; build Finished; `cargo test --workspace` 0 failed; ui lint+build OK;
  `git diff --exit-code -- ui/src/bindings` clean). Full log:
  `docs/audits/pr9-final-suite.log` (local).
